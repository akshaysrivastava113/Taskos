...
export default class TodoModal extends React.Component {
    ...
    renderTodo = todo => {
        ...
        return (
            <View style={styles.todoContainer}>
                <TouchableOpacity>
                    <Ionicons name={todo.completed ? "ios-square":"ios-square-outline"} size={24} color={colors.greyTwo} style={{width: 32}}/>
                </TouchableOpacity>
                <Text style={[styles.todo, {color: todo.completed ? colors.greyTwo:colors.black, textDecorationLine: todo.completed ? 'line-through':'none'}]}>{todo.title}</Text>
            </View>
        )
    ...
    render() {
        ...
        const completedCount = this.state.todos.filter(todos => todos.completed).length;
        ...
        return (
            <SafeAreaView style={styles.container}>
                ...
                <View style={[styles.section,styles.header, {borderColor: this.state.color}]}>
